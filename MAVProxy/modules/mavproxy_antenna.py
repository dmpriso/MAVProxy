...
class AntennaModule(mp_module.MPModule):
    ...
    def cmd_antenna(self, args):
        ...
        if args[0] == 'use' and args[1] in [ 'servotracker', 'none' ]:
            modname = args[1]
            if modname == 'none':
                self.servotracker = None
                print("Not using external tracker")
            else:
                self.servotracker = self.module(modname)
                if self.servotracker is not None:
                    print(f"Using external tracker {modname}")
                else:
                    print(f"Module {modname} could not be found")
        else:
    ...
    def point_antenna(self):
        ...
        if (time.time() - self.last_announce) > 10 and (
                abs(self.last_bearing - bearing) > 5 or
                abs(self.last_elevation - elevation) > 5
            ):
            self.last_bearing = bearing
            self.last_elevation = elevation
            self.last_announce = time.time()
            self.say(f'Antenna bearing {int(bearing + 0.5)} elevation {int(elevation + 0.5)}' + \
                f' vehicle distance {self.vehicle_distance} altitude {self.vehicle_alt}')

        if self.servotracker is not None:
            self.servotracker.slew_to(bearing, elevation)
    ...
    def mavlink_packet(self, m):
        ...
        if m.get_type() == 'GPS_RAW' and not m.fix_type == 0 and is_valid_pos(m.lat, m.lon) == True:
            self.process_vehicle_pos(m.lat, m.lon)
        elif m.get_type() == 'GPS_RAW_INT' and not m.fix_type == 0 and is_valid_pos(m.lat, m.lon) == True:
            self.process_vehicle_pos(m.lat / 1.0e7, m.lon / 1.0e7)
        elif m.get_type() == 'GLOBAL_POSITION_INT' and is_valid_pos(m.lat, m.lon) == True:
